...
function Detail({ data, repositoryId }: { data: ProjectDetail; repositoryId: string }) {
  ...
  return <>
    ...
    <section className="metric-grid"><Metric label="Commits" value={data.activity.commits} /><Metric label="Pull requests" value={data.activity.pullRequests} /><Metric label="Reviews" value={data.activity.reviews} /><Metric label="Issues" value={data.activity.issues} /><Metric label="Additions" value={`+${formatNumber(data.activity.additions)}`} /><Metric label="Deletions" value={`−${formatNumber(data.activity.deletions)}`} /></section>
    ...
    <section className="dashboard-section"><span className="card-kicker">Activity</span><h2>Commit activity over time</h2><DrilldownTimeChart points={data.activity.timeline.map(point => ({ month: point.month, value: point.commits }))} valueLabel="Commits" emptyText="No commit activity recorded." /></section>
...
function Rationale({ value }: { value?: Record<string, unknown> }) { if (!value) return <p className="empty-state">No assessment available.</p>; return <dl className="rationale-list">{Object.entries(value).slice(0, 8).map(([key, item]) => <div key={key}><dt>{humanize(key)}</dt><dd>{String(item)}</dd></div>)}</dl> }

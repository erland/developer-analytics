...
export function ProjectTypeViews() {
  ...
  if (projectTypes.status === 'loading') {
    return <section className="dashboard-loading" aria-live="polite"><div className="loading-indicator" aria-hidden="true" /><p>Loading project types…</p></section>
  }

  if (projectTypes.status === 'error') {
    return <section className="dashboard-error" role="alert"><h2>Project type data could not be loaded.</h2><p>{projectTypes.error}</p></section>
  }

  return <>
    <div className="view-toolbar">
      <div><p className="eyebrow">Project classification</p><h2>Project types</h2></div>
      <span className="inventory-count">{projectTypes.data.length} categories</span>
    </div>

    {projectTypes.data.length === 0 ? (
      <section className="empty-inventory"><h3>No project categories yet.</h3><p>Run project classification before viewing project-type trends.</p></section>
    ) : (
        ...
        <section className="technology-list" aria-label="Project type list">
          {projectTypes.data.map((item) => (
            <button
              ...
              className={`technology-list-item ${selected?.categoryKey === item.categoryKey ? 'technology-list-item-active' : ''}`}
              onClick={() => setSelectedKey(item.categoryKey)}
            >
              <div><strong>{item.categoryName}</strong><span>{item.projectCount} projects</span></div>
              <div className="technology-list-meta"><strong>{item.activityCount}</strong><span>activity</span></div>
            </button>
          ))}
        </section>
...
function ProjectTypeDetail({ item, onOpenProject }: { item: ProjectTypeView; onOpenProject: (id:string)=>void }) {
  return <div className="technology-detail">
    ...
    <section className="metric-grid">
      ...
      <Metric label="Timeline months" value={item.timeline.length} />
    </section>
    ...
    <section className="dashboard-section">
      ...
      <DrilldownTimeChart
        points={item.timeline.map(point => ({
        ...
        valueLabel="Activity"
        emptyText="No activity timeline available."
      />
    </section>
          ...
          {item.representativeProjects.map((project) => (
            <article className="project-row" key={project.repositoryId}>
              <div>
                ...
                <p>{ownershipLabel(project.ownershipRelation)} · {project.visibility.toLowerCase()} · {project.contributionCount} contribution{project.contributionCount === 1 ? '' : 's'}</p>
              </div>

    ...
    {technologies.data.length === 0 ? <section className="empty-inventory"><h3>No technology assessments yet.</h3><p>Collect repository evidence and recalculate technology assessments first.</p></section> : (
      <div className="technology-layout">
        <section className="technology-list" aria-label="Technology list">
          {technologies.data.map((technology) => <button type="button" key={technology.technologyKey} className={`technology-list-item ${selected?.technologyKey === technology.technologyKey ? 'technology-list-item-active' : ''}`} onClick={() => setSelectedKey(technology.technologyKey)}><div><strong>{technology.technologyName}</strong><span>{humanize(technology.technologyCategory)}</span></div><div className="technology-list-meta"><strong>{technology.evidenceLevel}</strong><span>{technology.projectCount} projects</span></div></button>)}
        </section>

...
type LineStatisticConsistency = {
  completeCommitCount: number
  matchingCommitCount: number
  mismatchingCommitCount: number
  commitAdditions: number
  commitDeletions: number
  fileAdditions: number
  fileDeletions: number
  netLineDifference: number
}

type DiagnosticState =
  | { status: 'idle' | 'loading' }
  | { status: 'ready'; data: LineStatisticConsistency }
...
export function diagnosticSummary(data: LineStatisticConsistency) {
  if (data.mismatchingCommitCount === 0 && data.netLineDifference === 0) return 'Commit and file line statistics are consistent.'
  return `${data.mismatchingCommitCount.toLocaleString()} commits differ · ${formatSigned(data.netLineDifference)} net-line difference`
}

export function SyncMonitoringPanel() {
  const monitoring = useSyncMonitoring()
  const [retryMessage, setRetryMessage] = useState<string | null>(null)
  const [diagnostics, setDiagnostics] = useState<DiagnosticState>({ status: 'idle' })
  ...
  async function loadDiagnostics() {
    if (diagnostics.status !== 'idle') return
    setDiagnostics({ status: 'loading' })
    try {
      const response = await fetch('/api/me/activity/line-stat-consistency', { credentials: 'include', headers: { Accept: 'application/json' } })
      if (!response.ok) throw new Error(`Diagnostics failed with HTTP ${response.status}`)
      setDiagnostics({ status: 'ready', data: await response.json() as LineStatisticConsistency })
    } catch (error) {
      setDiagnostics({ status: 'error', error: error instanceof Error ? error.message : 'Unable to load diagnostics' })
    }
  }
  ...
  return <details className={`dashboard-section secondary-details sync-monitoring-details sync-state-${headline.kind}`} aria-labelledby="sync-progress-heading">
    ...
    <div className="secondary-details-content">
      ...
      <details className="sync-resolved-issues" onToggle={(event) => { if (event.currentTarget.open) void loadDiagnostics() }}>
        <summary>Data consistency diagnostics</summary>
        <p className="settings-intro">Read-only comparison of commit-level line statistics and summed file-level Change Type statistics for fully classified commits.</p>
        {diagnostics.status === 'idle' || diagnostics.status === 'loading' ? <p className="empty-state">Loading diagnostics…</p> : null}
        {diagnostics.status === 'error' ? <p className="sync-error-text">{diagnostics.error}</p> : null}
        {diagnostics.status === 'ready' ? <>
          <div className="sync-current" role="status"><strong>{diagnosticSummary(diagnostics.data)}</strong><span>{fmt(diagnostics.data.matchingCommitCount)} of {fmt(diagnostics.data.completeCommitCount)} complete commits match exactly.</span></div>
          <div className="sync-status-grid">
            <Status label="Complete commits" value={diagnostics.data.completeCommitCount} />
            <Status label="Matching" value={diagnostics.data.matchingCommitCount} />
            <Status label="Mismatching" value={diagnostics.data.mismatchingCommitCount} />
          </div>
          <div className="sync-current"><strong>Commit-level net lines</strong><span>{formatSigned(diagnostics.data.commitAdditions - diagnostics.data.commitDeletions)}</span></div>
          <div className="sync-current"><strong>File-level net lines</strong><span>{formatSigned(diagnostics.data.fileAdditions - diagnostics.data.fileDeletions)}</span></div>
          <div className="sync-current"><strong>Difference</strong><span>{formatSigned(diagnostics.data.netLineDifference)}</span></div>
        </> : null}
      </details>
...
function Status({ label, value }: { label: string; value: number }) { return <div><span>{label}</span><strong>{fmt(value)}</strong></div> }
...
const formatSigned = (value: number) => `${value >= 0 ? '+' : '−'}${fmt(Math.abs(value))}`

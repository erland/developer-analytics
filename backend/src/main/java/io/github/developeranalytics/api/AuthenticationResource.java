...
public class AuthenticationResource {
    ...
    public Response callback(@QueryParam("code") String code,@QueryParam("state") String state) throws Exception {
        if(code==null || state==null) throw new BadRequestException("Missing GitHub OAuth callback parameters");
        AuthenticationService.LoginResult result=auth.finishGitHubLogin(code,state);
        return Response.seeOther(URI.create(frontendUrl))
            .header(HttpHeaders.SET_COOKIE,auth.sessionCookie(result.token(),result.expiresAt())).build();
    }

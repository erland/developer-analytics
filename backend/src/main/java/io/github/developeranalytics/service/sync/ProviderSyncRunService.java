...
public class ProviderSyncRunService {
    ...
    @Transactional
    public void recordApiUsage(UUID runId, int requests, Map<String,Integer> requestsByEndpoint) {
        if (runId == null || requests <= 0) return;
        runs.findByIdForUpdate(runId).ifPresent(run -> run.addApiUsage(requests, requestsByEndpoint));
    }
    ...
    public Summary summarize(ProviderSyncRun run) {
        ...
        int seen = 0, created = 0, updated = 0;
        ...
        return new Summary(run.getId(), run.getProvider(), run.getSyncMode(), run.getStatus(),
                ...
                seen, created, updated, run.getApiRequestCount(), run.getApiRequestsByEndpoint(),
    ...
    public UUID payloadRunId(io.github.developeranalytics.domain.job.BackgroundJob job) {
        ...
        try { return UUID.fromString(value.toString()); } catch (IllegalArgumentException ignored) { return null; }
    }

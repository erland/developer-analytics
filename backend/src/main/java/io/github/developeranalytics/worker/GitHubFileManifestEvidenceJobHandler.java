...
import io.github.developeranalytics.service.technology.FileManifestEvidenceService;
...
@ApplicationScoped
public class GitHubFileManifestEvidenceJobHandler implements BackgroundJobHandler {

    public static final String JOB_TYPE = "GITHUB_FILE_MANIFEST_EVIDENCE";

    @Inject
    SourceRepositoryRepository repositories;

    @Inject
    FileManifestEvidenceService evidence;
    ...
    public void handle(BackgroundJob job) throws Exception {
        ...
        if (job.getUser() == null || repositoryId == null) {
            throw new IllegalStateException(
                    "File/manifest evidence job requires user and repositoryId"
            );
        }

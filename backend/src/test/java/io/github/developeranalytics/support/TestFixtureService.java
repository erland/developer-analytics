...
public class TestFixtureService {
    ...
    public AppUser createUserWithSession(String externalId, String login, String displayName, String rawSessionToken) {
        AppUser user = AppUser.create();
        entityManager.persist(user);
        ProviderIdentity identity = new ProviderIdentity(user, "github", externalId, login, displayName);
        entityManager.persist(identity);
        entityManager.persist(new UserSession(user, CryptoTokens.sha256(rawSessionToken), OffsetDateTime.now().plusHours(1)));
        entityManager.flush();
        return user;
    }

    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public SourceRepository createRepository(AppUser user, String externalRepositoryId, String ownerLogin, String name) {
        AppUser managedUser = entityManager.getReference(AppUser.class, user.getId());
        SourceRepository repository = new SourceRepository(managedUser, "github", externalRepositoryId, ownerLogin, name);
    ...
    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public Contribution createContribution(AppUser user, SourceRepository repository, String providerContributionId,
                                           Contribution.Type type, OffsetDateTime occurredAt) {
        AppUser managedUser = entityManager.getReference(AppUser.class, user.getId());
        SourceRepository managedRepository = entityManager.getReference(SourceRepository.class, repository.getId());
        Contribution contribution = new Contribution(managedUser, managedRepository, "github", providerContributionId, type, occurredAt);
    ...
    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public Contribution createCommitWithLineStatistics(AppUser user, SourceRepository repository,
                                                       String providerContributionId, OffsetDateTime occurredAt,
                                                       int additions, int deletions, int changedFiles) {
        AppUser managedUser = entityManager.getReference(AppUser.class, user.getId());
        SourceRepository managedRepository = entityManager.getReference(SourceRepository.class, repository.getId());
        Contribution contribution = new Contribution(managedUser, managedRepository, "github", providerContributionId,
                Contribution.Type.COMMIT, occurredAt);
    ...
    public ProviderConnection createGitHubConnection(AppUser user, String externalId, String login, String displayName) {
        ...
        ProviderIdentity identity = entityManager.createQuery(
                        "select i from ProviderIdentity i where i.user.id = :userId and i.provider = :provider", ProviderIdentity.class)
                .setParameter("userId", user.getId()).setParameter("provider", "github").getResultStream().findFirst()
                .orElseGet(() -> { ProviderIdentity created = new ProviderIdentity(managedUser, "github", externalId, login, displayName); entityManager.persist(created); return created; });
        ProviderConnection connection = new ProviderConnection(managedUser, identity, "github");
    ...
    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public ExternalClientToken createExternalClientToken(AppUser user, String name, String rawToken,
                                                         Set<ExternalClientToken.Scope> scopes,
                                                         ExternalClientToken.PrivacyScope privacyScope) {
        AppUser managedUser = entityManager.getReference(AppUser.class, user.getId());
        ExternalClientToken token = new ExternalClientToken(managedUser, name, CryptoTokens.sha256(rawToken), scopes, privacyScope);
